...
- packages/base/src/* : 기본 Extension/Connector/Tool 묶음 (`self-restart` Tool: request(restart signal), `telegram` Tool: send/edit/delete/react/setChatAction + parseMode, `slack` Tool: send/read/edit/delete/react, `slack` Connector: webhook port/path 설정 지원, `telegram-polling` bot-origin self-feedback 필터 포함)
