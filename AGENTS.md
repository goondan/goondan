...
- packages/runtime/src/* : 오케스트레이터 런타임/Config/LiveConfig (Bundle validate 시 Tool/Extension/Connector `spec.entry` 파일 존재까지 fail-fast 검증, Turn/Step/Tool RuntimeEvent를 `messages/runtime-events.jsonl`에 append-only 영속화)
...
- packages/base/src/* : 기본 Extension/Connector/Tool 묶음 (`wait` Tool: seconds(지정 초 대기), `self-restart` Tool: request(restart signal), `telegram` Tool: send/edit/delete/react/setChatAction/downloadFile + parseMode, `slack` Tool: send/read/edit/delete/react/downloadFile, `slack` Connector: webhook port/path 설정 + 첨부 image/file 참조 텍스트 보강 지원, `telegram-polling` bot-origin self-feedback 필터 + photo/image document file_id 메타 전달 포함, `message-window`/`message-compaction`의 tool-call/tool-result 정합성 보정)
