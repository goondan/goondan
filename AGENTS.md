...
- docs/specs/runtime.md : **[v0.0.3]** Runtime 실행 모델 스펙 (배경/설계 동기, 핵심 규칙 통합, Orchestrator 상주 프로세스, Process-per-Agent, IPC 메시지 브로커, **Reconciliation Loop**, **Graceful Shutdown Protocol**, Turn/Step, Message 이벤트 소싱, Extension `ctx.agents` 경로, request 순환 호출 감지, Edit & Restart, Observability)
