...
Goondan은 의식과 무의식의 관계로 에이전트 구성을 설명하는 **‘마음’ 컨셉**에서 아이디어를 가져왔습니다.

의식에 해당하는 에이전트는 사용자의 목표를 따라 모델을 호출하고, 필요한 도구와 서브에이전트를 명시적으로 사용합니다. 무의식에 해당하는 보조 기능은 구성에 정한 조건에 따라 참여합니다. 예를 들어 모델을 호출하기 전에 관련 지식을 찾아 맥락을 추가하거나, 응답이 나온 뒤 다른 에이전트가 문장을 다듬습니다. 중심 에이전트는 이런 보조 기능의 구현이나 서로의 이름을 알 필요가 없습니다.

전체를 다시 조합할 수 있다는 점도 이 컨셉에서 가져왔습니다. 분석 에이전트와 편집 에이전트를 직렬로 연결한 구성을 더 큰 구성의 한 에이전트로 사용할 수 있습니다. 보조 에이전트도 자신의 도구와 훅을 가질 수 있습니다. 기능을 붙이거나 제거할 때 중심 에이전트의 구현을 바꾸는 부담을 줄이는 것이 목적입니다.

Goondan에서 이 생각은 다음과 같이 표현됩니다.
...
훅은 값을 받아 결과를 반환하고, 목록에 적힌 순서가 실행 순서가 됩니다. 확장은 다른 확장의 실행 순서를 지정하지 않습니다. 어떤 보조 기능을 사용할지, 어느 모델을 연결할지는 구성을 작성하는 쪽이 결정합니다.
