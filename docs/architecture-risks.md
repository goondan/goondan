...
### 6. 세션 삭제 시점은 호스트가 정한다

상태 `수용` · 시점 `규모 확대 시` · 범위 TypeScript, Python

현재 설계에서 감수하는 내용

`sessions.delete`는 지정한 세션과 `<세션>#` 접두사의 파생 세션에 속한 확장 인스턴스, 비동기 훅 상태와 대화를 정리한다(`packages/core/src/runtime.ts`, `python/goondan/goondan/runtime.py`). 세션의 업무상 수명과 보존 기간은 호스트만 알 수 있으므로, 자동 만료 시점은 런타임이 정하지 않는다.

호스트가 알아야 할 점

호스트는 사용자 세션 종료, 로그아웃이나 TTL 만료 같은 수명 정책에 따라 `sessions.delete`를 호출한다. 삭제 실패를 재시도하고 영속 대화 저장소의 실제 삭제 여부를 관측한다. 승인 작업은 세션 상태와 별도 수명을 가지므로 작업 보존과 삭제 정책도 호스트가 운영 요건에 맞춰 정한다.
...
| 실행 이벤트만으로 부모 실행과 최상위 요청을 연결할 수 없었다. | 이벤트와 `runs`에 `instance`, `parentInstance`, `parentTurnId`, `rootTurnId`가 있으며, 승인 작업은 같은 값을 저장하여 비동기 실행과 복구 뒤의 완료 전달에도 전파한다(`spec/goondan.md`, `packages/core/src/runtime.ts`, `python/goondan/goondan/runtime.py`). | 하위 실행이 부모 식별자를 잃거나 작업 저장·복구와 완료 전달에서 최상위 턴 식별자를 새로 만든다. |
