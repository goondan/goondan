# Goondan Bundle 요구사항 (Git 기반)

본 문서는 Goondan 생태계에서 **Bundle을 Git 기반으로 식별/다운로드/의존성 해석**하기 위한 요구사항을 정의한다. 
핵심 목적은 **npm 패키지 개념과 Goondan 번들 개념의 혼선을 제거**하고, 번들 자체(스크립트, YAML 정의, 비-Node 런타임 코드 등)를 **그대로 배포/사용**하는 것이다.

---

## 1. 목표

1. Bundle은 **실행 가능한 리소스 묶음**(Tool/Extension/Connector 정의 + 스크립트 파일)을 의미한다.
2. npm은 **선택적 호스팅/메타데이터**로만 사용 가능하며, **필수 의존성 관리 도구가 아니다**.
3. Bundle은 **Git 경로**로 식별되고, **bundle.yaml이 있는 폴더 전체가 다운로드**되어야 한다.
...
- `<host>`: 예: `github.com`
- `<org>/<repo>`: Git 리포지토리
- `<path>`: 리포 내 Bundle Root 경로 (생략 불가, root 번들은 `/`로 취급 가능)
...
2. Bundle Ref는 **Git fetch 가능한 주소**로 해석되어야 한다(MUST).

---

## 4. 다운로드 및 캐시 규칙

1. Bundle을 해석할 때, **Bundle Root 전체 디렉터리 트리를 다운로드**한다(MUST).
...
2. `metadata.name`은 Bundle의 식별명으로 사용된다(MUST).
3. `spec.dependencies`는 Bundle Ref 목록이다(MAY).
...
단, `spec.entry` 경로는 **Bundle Root 기준 상대 경로**로 해석한다(MUST).
...
1. Bundle을 로드하면 `spec.dependencies`를 **재귀적으로 해석**한다(MUST).
2. 로드 순서는 **의존성 → 현재 번들** 순으로 처리한다(SHOULD).
3. 하나의 Bundle 안에서는 `spec.include`에 나열된 **순서대로 리소스를 로드**한다(SHOULD).
...
규칙:
1. `bundle`을 지정하면 해당 Bundle Root에서만 리소스를 탐색한다(MUST).
...
1. Bundle 다운로드 후 **Fingerprint(해시)**를 계산한다(SHOULD).
2. `bundle lock`은 Bundle Ref와 해시/커밋 정보를 저장해 **재현 가능한 로딩**을 보장한다(SHOULD).
