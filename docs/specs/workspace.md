# Goondan Workspace 및 Storage 모델 스펙 (v0.8)
...
function createInstanceStatePaths(
  ...
  return {
    ...
    agent(agentName: string): AgentStatePaths {
      ...
      return {
        root: agentRoot,
        messagesLog: path.join(agentRoot, 'messages', 'llm.jsonl'),
...
## 6. LLM Message Log 스키마

Runtime은 AgentInstance별로 LLM 메시지를 append-only JSONL로 기록해야 한다(MUST).

### 6.1 파일 위치

```
<goondanHome>/instances/<workspaceId>/<instanceId>/agents/<agentName>/messages/llm.jsonl
```
...
```json
{"type":"llm.message","recordedAt":"2026-02-01T12:34:56.789Z","instanceId":"default-cli","instanceKey":"cli","agentName":"planner","turnId":"turn-abc123","stepId":"step-xyz789","stepIndex":0,"message":{"role":"user","content":"현재 디렉터리의 파일 목록을 보여줘"}}
{"type":"llm.message","recordedAt":"2026-02-01T12:34:57.123Z","instanceId":"default-cli","instanceKey":"cli","agentName":"planner","turnId":"turn-abc123","stepId":"step-xyz789","stepIndex":0,"message":{"role":"assistant","toolCalls":[{"id":"call_001","name":"file.list","arguments":{"path":"."}}]}}
{"type":"llm.message","recordedAt":"2026-02-01T12:34:57.456Z","instanceId":"default-cli","instanceKey":"cli","agentName":"planner","turnId":"turn-abc123","stepId":"step-xyz789","stepIndex":0,"message":{"role":"tool","toolCallId":"call_001","toolName":"file.list","output":["README.md","package.json","src/"]}}
{"type":"llm.message","recordedAt":"2026-02-01T12:34:58.789Z","instanceId":"default-cli","instanceKey":"cli","agentName":"planner","turnId":"turn-abc123","stepId":"step-xyz790","stepIndex":1,"message":{"role":"assistant","content":"현재 디렉터리에는 README.md, package.json, src/ 폴더가 있습니다."}}
```

### 6.4 규칙

1. 각 레코드는 **단일 JSON 라인**이어야 한다(MUST).
2. 레코드는 **append-only**로 기록되어야 한다(MUST).
3. 파일은 **UTF-8 인코딩**이어야 한다(MUST).
4. `recordedAt`은 **ISO8601 형식**이어야 한다(MUST).
...
```json
{"type":"swarm.event","recordedAt":"2026-02-01T12:00:00.000Z","kind":"swarm.created","instanceId":"default-cli","instanceKey":"cli","swarmName":"default"}
{"type":"swarm.event","recordedAt":"2026-02-01T12:00:01.000Z","kind":"agent.created","instanceId":"default-cli","instanceKey":"cli","swarmName":"default","agentName":"planner"}
{"type":"swarm.event","recordedAt":"2026-02-01T12:30:00.000Z","kind":"changeset.committed","instanceId":"default-cli","instanceKey":"cli","swarmName":"default","data":{"changesetId":"cs-001","baseRef":"git:abc123","newRef":"git:def456"}}
```
...
```json
{"type":"agent.event","recordedAt":"2026-02-01T12:34:56.000Z","kind":"turn.started","instanceId":"default-cli","instanceKey":"cli","agentName":"planner","turnId":"turn-abc123"}
{"type":"agent.event","recordedAt":"2026-02-01T12:34:56.100Z","kind":"step.started","instanceId":"default-cli","instanceKey":"cli","agentName":"planner","turnId":"turn-abc123","stepId":"step-xyz789","stepIndex":0}
{"type":"agent.event","recordedAt":"2026-02-01T12:34:57.000Z","kind":"toolCall.started","instanceId":"default-cli","instanceKey":"cli","agentName":"planner","turnId":"turn-abc123","stepId":"step-xyz789","stepIndex":0,"data":{"toolCallId":"call_001","toolName":"file.list"}}
{"type":"agent.event","recordedAt":"2026-02-01T12:34:57.500Z","kind":"toolCall.completed","instanceId":"default-cli","instanceKey":"cli","agentName":"planner","turnId":"turn-abc123","stepId":"step-xyz789","stepIndex":0,"data":{"toolCallId":"call_001","toolName":"file.list","durationMs":500}}
```
...
console.log(paths.agentMessagesLogPath('default-cli', 'planner'));
// => "/Users/alice/.goondan/instances/a1b2c3d4e5f6/default-cli/agents/planner/messages/llm.jsonl"
...
```typescript
// LLM 메시지 로그 작성
const messageLog = new JsonlWriter<LlmMessageLogRecord>(
  paths.agentMessagesLogPath('default-cli', 'planner')
);
...
async function initializeInstanceState(
  ...
  for (const agentName of agents) {
    await fs.mkdir(
      path.dirname(paths.agentMessagesLogPath(instanceId, agentName)),
      { recursive: true }
    );
    await fs.mkdir(
...
3. LLM 메시지 로그는 append-only JSONL로 기록되어야 한다.
4. Swarm/Agent 이벤트 로그는 append-only JSONL로 기록되어야 한다.
5. OAuth 저장소의 비밀값은 at-rest encryption을 적용해야 한다.
