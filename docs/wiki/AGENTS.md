...
- `how-to/` - 특정 문제 해결 실용 가이드
  ...
  - `multi-agent-patterns.md` / `multi-agent-patterns.ko.md` - 멀티 에이전트 패턴: agents 도구 설정, request(동기 요청-응답) 패턴, send(비동기 fire-and-forget) 패턴, spawn(인스턴스 준비) 패턴, `turn`/`step` 미들웨어의 `ctx.agents.request/send` 패턴, list/catalog로 에이전트 발견, Coordinator+Specialist 실전 시나리오(brain-persona 샘플), instanceKey 공유/격리 전략, 자동 스폰 동작, request vs send 선택 기준 (EN/KO)
- `explanation/` - 핵심 개념 이해
  ...
  - `extension-pipeline.md` / `extension-pipeline.ko.md` - Extension & Pipeline 아키텍처: Onion 모델, turn/step/toolCall 3종 미들웨어, `turn`/`step` 컨텍스트의 `ctx.agents` 호출 경로, ConversationState 이벤트 소싱, ExtensionApi 5개 영역, register(api) 패턴, 실전 패턴(Skill/Compaction/Logging/MCP) (EN/KO)
...
- `reference/` - API/스키마/CLI 정보 조회
  ...
  - `extension-api.md` / `extension-api.ko.md` - Extension API 레퍼런스: register(api) 엔트리 패턴, ExtensionApi 5개 영역(pipeline/tools/state/events/logger), PipelineRegistry 인터페이스, TurnMiddleware/StepMiddleware/ToolCallMiddleware 시그니처 및 컨텍스트 필드(`turn`/`step`의 `ctx.agents` 포함), ExtensionToolsApi 동적 도구 등록, ExtensionStateApi 영속 상태 관리, ExtensionEventsApi 이벤트 버스, 표준 런타임 이벤트 목록, Onion 실행 모델 다이어그램, 보조 타입(ConversationState/MessageEvent/Message/ExecutionContext) (EN/KO)
