...
4. 코어의 지속 가능한 상태는 세션별 append 전용 저널 하나에 기록하며, CLI는 같은 저장소 계약의 JSONL 구현을 제공합니다.
...
- 실행 식별자는 `sessionId`, `turnId`, `instance`, `executionId`, `inputId`의 범위를 섞지 않으며 원인은 `parentExecutionId` 또는 `operationId`로 기록합니다.
