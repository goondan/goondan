...
- `base`: 기본 Tool/Extension/Connector 구현 (`self-restart` Tool: request(restart signal), `telegram` Tool: send/edit/delete/react/setChatAction + parseMode, `slack` Tool: send/read/edit/delete/react, `slack` Connector: webhook port/path 설정 지원, `telegram-polling`의 bot-origin self-feedback 필터 포함)
