...
- `cli(runtime path)`: runtime runner 모듈 경로는 `@goondan/runtime/package.json` module resolution(`require.resolve`)을 우선 사용하고, 글로벌 설치 레이아웃(Bun/npm) fallback 탐색을 함께 적용
