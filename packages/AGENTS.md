...
- `base`: 기본 Tool/Extension/Connector 구현 (`wait` Tool: seconds(지정 초 대기), `self-restart` Tool: request(restart signal), `telegram` Tool: send/edit/delete/react/setChatAction/downloadFile + parseMode, `slack` Tool: send/read/edit/delete/react/downloadFile, `slack` Connector: webhook port/path 설정 + 첨부 image/file 참조 텍스트 보강 지원, `telegram-polling`의 bot-origin self-feedback 필터 + photo/image document file_id 메타 전달 포함, 메시지 정책 Extension이 tool-call/tool-result 짝 정합성을 보존)
