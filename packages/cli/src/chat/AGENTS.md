...
4. 세션 파일은 버전 2 형식으로 대화 식별자별 묶음 아래에 에이전트 경로별 메시지를 보존합니다. 버전 1 파일은 세션 자신의 대화로 읽고 다음 쓰기에서 버전 2로 올리며, 파일은 원자적으로 교체합니다.
5. 바인딩 모듈이 없으면 구성과 중첩 구성이 선언한 모든 모델 이름을 고른 어댑터 하나에 묶습니다.
