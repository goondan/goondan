...
export interface HookContext { execution: ExecutionControl; agent: string; conversationId: string; turnId: string; step?: number; retryCount: number; input: Json; conversation: readonly Message[]; signal: AbortSignal; agents: { run(name: string, value: Json, options?: { conversation?: Message[]; signal?: AbortSignal }): Promise<AgentRunResult> }; model: { run(messages: Message[], options?: { signal?: AbortSignal }): Promise<AgentRunResult> }; render(template: string, variables: Record<string, Json>): Promise<string>; message: { user(text: string, extra?: MessageExtra): Message; system(text: string, extra?: MessageExtra): Message }; append(...items: Message[]): Append; log: Logger }
