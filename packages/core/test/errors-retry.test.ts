...
import {
  createRuntime, MemoryConversationStore,
...
describe("the retry limit", () => {
  it("follows three retries by default, so a failing model is called four times", async () => {
    ...
    const seen: Json[] = [];
    const runtime = createRuntime({ agents: { main: { model: "m", hooks: { error: [{ name: "again", fn: "again" }] } } } }, {
  ...
  it("refuses a retry limit that is not a whole number of zero or more", () => {
    expect(() => createRuntime({ agents: { a: { model: "m" } } }, { directory: ".", models: {}, maxRetries: -1 })).toThrow(TypeError);
    expect(() => createRuntime({ agents: { a: { model: "m" } } }, { directory: ".", models: {}, maxRetries: 1.5 })).toThrow(TypeError);
  });
...
describe("a tool retry", () => {
  ...
  it("carries the failed call of the tool location in the execution error", async () => {
    ...
    const runtime = createRuntime({ agents: { main: { model: "m", tools: ["act"] } } }, { directory: ".", models: { m: model }, tools: { act } });

    const error: unknown = await runtime.runTurn("go", { conversationId: "c" }).catch((issue: unknown) => issue);
