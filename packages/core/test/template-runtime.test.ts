...
import { createGoondan, defineExtension, loadConfigSync, type Json, type ModelResult } from "../src/index.ts";
...
describe("template variables of each declaration site", () => {
  it("gives a system block params, tools, agent and model", async () => {
    ...
    const seen = recorder();
    const runtime = createGoondan(loadConfigSync(root), {
      models: { m: seen.model },
  ...
  it("applies an input template to JSON parts and leaves text parts unchanged", async () => {
    const root = workspace({
      "goondan.yaml": "agents:\n  main: {model: m, input: {template: ./in.md}}\n",
      "in.md": "[{{ text | default('-') }}|{{ a | default('-') }}]",
    });
    const seen = recorder();
    const runtime = createGoondan(loadConfigSync(root), { models: { m: seen.model } });
    await runtime.run({ a: 1 }, { sessionId: "c" });
    expect(seen.user).toBe("[-|1]");
    await runtime.run("plain", { sessionId: "d" });
    expect(seen.user).toBe("plain");
    await runtime.close();
  });

  it("rejects an inline input template because input-stage values are message arrays", async () => {
    const root = workspace({
    ...
    const seen = recorder();
    const runtime = createGoondan(loadConfigSync(root), { models: { m: seen.model } });
    const error: unknown = await runtime.run("in", { sessionId: "c" }).catch((reason: unknown) => reason);
  ...
  it("prints the assistant message an output hook template receives", async () => {
    ...
    const runtime = createGoondan(loadConfigSync(root), { models: { m: echo } });
  ...
  it("fails the hook stage when an inline hook template cannot render", async () => {
    ...
    const runtime = createGoondan(loadConfigSync(root), { models: { m: echo } });
    const error: unknown = await runtime.run("x", { sessionId: "c" }).catch((reason: unknown) => reason);
    expect(failure(error)).toEqual({ where: "input", codes: ["hook_error"] });
    await runtime.close();
  });
  ...
  it("reads the templates of a plain document from the directory the host names", async () => {
    ...
    const seen = recorder();
    const runtime = createGoondan(document, { models: { m: seen.model }, directory: root });
    await runtime.run("x", { sessionId: "c" });
...
describe("template rendering inside a runtime", () => {
  it("fails the input stage when the input template cannot render", async () => {
    ...
    const runtime = createGoondan(loadConfigSync(root), { models: { m: echo } });
    const error: unknown = await runtime.run({}, { sessionId: "c" }).catch((reason: unknown) => reason);
  ...
  it("renders a loaded template from a hook by absolute or configuration-relative path", async () => {
    ...
    const runtime = createGoondan(loadConfigSync(root), { models: { m: echo }, extensions: { probe } });
    await runtime.run("x", { sessionId: "c" });
  ...
  it("reuses the templates of a load result and never reads a file again", async () => {
    ...
    let system = "";
    const runtime = createGoondan(loaded, { models: { m: { async generate(input): Promise<ModelResult> {
