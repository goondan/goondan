...
- `src/pipeline/*`: turn/step/toolCall 미들웨어 레지스트리 및 onion 체이닝 (`turn`/`step` 컨텍스트의 `ctx.agents` 제공 포함)
...
- `src/runner/runtime-runner.ts`: watch/tool 신호 기반 self-restart 시 shutdown(Connector 종료) 이후 replacement runner 기동으로 포트 충돌을 회피, Extension `ctx.agents`를 기존 IPC 경로에 연결, request 순환 호출 감지
