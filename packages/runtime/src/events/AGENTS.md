...
- RuntimeEvent 타입 계약은 `@goondan/types`가 SSOT로 소유하며, 이 모듈은 re-export + runtime 고유 구현만 제공한다.
- 이벤트 구독/발행 메커니즘(EventBus)을 런타임 내부에 캡슐화한다.

## 구조적 결정

1. 타입 정의는 `@goondan/types`에서 re-export만 한다 (자체 타입 정의 없음).
...
- 이 모듈에서 RuntimeEvent 관련 새 타입을 정의하지 않는다. 타입 추가가 필요하면 `@goondan/types`에서 한다.
