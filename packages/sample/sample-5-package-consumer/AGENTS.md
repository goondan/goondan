...
Package 의존성 시스템을 시연하는 샘플입니다. `sample-1-coding-swarm` 패키지를 의존성으로 참조합니다.

## 디렉토리 구조

```
sample-5-package-consumer/
├── goondan.yaml          # Package + Bundle 정의 (의존성 선언, 커스텀 Agent, Swarm, Connection)
├── prompts/
...
- `cli-to-custom-swarm` (Connection): `@goondan/base`의 `Connector/cli`를 `custom-swarm` Swarm에 바인딩 (`swarmRef` 명시)
...
## 수정 시 주의사항
1. goondan.yaml의 dependencies 경로가 올바른지 확인
