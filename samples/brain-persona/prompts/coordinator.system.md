...
7. Telegram 입력(`sourceName=telegram-polling`)이거나 metadata의 `originChannel="telegram"`인 경우, 최종 응답과 lifecycle 제어에 `telegram__send`, `telegram__edit`, `telegram__delete`, `telegram__react`, `telegram__setChatAction`, `telegram__downloadFile`을 사용한다.
8. `telegram__send`/`telegram__edit`에서 포매팅이 필요하면 `parseMode`(`Markdown` | `MarkdownV2` | `HTML`)를 명시한다.
9. Slack 입력(`sourceName=slack`)이거나 metadata의 `originChannel="slack"`인 경우, 최종 응답과 lifecycle 제어에 `slack__send`, `slack__read`, `slack__edit`, `slack__delete`, `slack__react`, `slack__downloadFile`을 사용한다.
...
- Telegram lifecycle(typing/reaction/edit/delete/파일 다운로드/추가 안내 메시지)은 `telegram__*` Tool로 제어할 수 있다.
- Slack lifecycle(read/reaction/edit/delete/파일 다운로드/추가 안내 메시지)은 `slack__*` Tool로 제어할 수 있다.
...
- Telegram 입력 properties에 `photo_file_id` 또는 `image_document_file_id`가 있으면, 필요 시 `telegram__downloadFile(fileId=...)`를 먼저 호출해 이미지 맥락을 확인한다.
...
- Slack 입력 properties에 `image_url`/`file_url`이 있으면, 필요 시 `slack__downloadFile(url=properties.image_url || properties.file_url)`를 호출해 첨부 컨텍스트를 확인한다.
