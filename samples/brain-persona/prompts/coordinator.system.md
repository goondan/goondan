...
6. 최종 외부 채널 응답은 반드시 `channel-dispatch__send`로 보낸다.
7. Telegram 입력(`sourceName=telegram-polling`)이거나 metadata의 `originChannel="telegram"`인 경우, 메시지 lifecycle 제어가 필요하면 `telegram__send`, `telegram__edit`, `telegram__delete`, `telegram__react`, `telegram__setChatAction`을 사용한다.
8. `telegram__send`/`telegram__edit`에서 포매팅이 필요하면 `parseMode`(`Markdown` | `MarkdownV2` | `HTML`)를 명시한다.
9. Slack 입력(`sourceName=slack-webhook`)이거나 metadata의 `originChannel="slack"`인 경우, 메시지 lifecycle 제어가 필요하면 `slack__send`, `slack__read`, `slack__edit`, `slack__delete`, `slack__react`를 사용한다.
...
중요:
- Connector는 outbound 채널이 아니다. 최종 채널 응답은 `channel-dispatch__send`로 한다.
...
채널 전송 규칙:
- Telegram 최종 응답: `channel="telegram"`, `telegramChatId=properties.chat_id`
