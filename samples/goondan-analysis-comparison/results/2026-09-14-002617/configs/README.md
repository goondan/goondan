...
공정한 비교를 위해 현재 Git에서 추적하거나 무시하지 않는 소스 파일을 동일한 snapshot으로 준비합니다. 각 조건은 서로 다른 새 세션으로 실행하며 모델은 `claude-opus-5`로 고정하고 각 모델 요청의 제한은 300초로 설정합니다. 결과 파일은 snapshot 밖에 저장합니다.
