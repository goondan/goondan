...
실행 가능한 호출은 현재 에이전트의 유효한 `tools` 목록으로 정한다. 호스트에 등록되어 있거나 같은 구성에 정의된 이름이라도 이 목록에 없으면 실행 오류다. 객체 항목은 `tool`과 `agent` 가운데 정확히 하나를 선언해야 하며, 이 구분에 따라 호스트 도구 또는 에이전트를 실행한다. 존재하지 않는 참조와 두 종류를 함께 선언한 항목은 구성 오류다.
...
비동기 훅은 `conversation` 단계에서 대화에 추가할 메시지를 준비하는 용도로 사용한다. 다른 단계에 `mode: async`를 선언하면 구성 오류다. 완료된 결과는 다음 안전한 대화 처리 지점에서 한 번 반영된다. 현재 턴이 끝나도 작업과 미반영 결과는 같은 런타임의 대화에 유지되어 다음 턴에서 사용할 수 있다. 런타임을 닫으면 남은 작업을 취소한다. 같은 에이전트와 대화에서 같은 훅 이름의 작업이 실행 중이거나 결과 반영을 기다리면 중복 실행을 예약하지 않는다. 비동기 결과는 현재 단계 값을 교체하거나 승인·재시도·실행 완료를 제어하는 용도로 사용할 수 없다.
...
도구 객체의 `approval: required` 또는 동기 `toolCall` 훅의 승인 결과는 도구 실행을 지속 가능한 승인 작업으로 바꾼다. 런타임은 안정적인 `operationId`를 만들고 `pending` 상태와 대기 중인 도구 결과를 저장한 뒤 현재 턴의 실행을 계속한다. 대기 작업은 에이전트 턴을 점유하지 않는다. 승인 요청과 저장된 작업은 `toolCall` 훅 처리가 끝난 호출 이름과 인수를 사용하며, 실행 대상은 해당 에이전트의 유효한 `tools` 목록에 있어야 한다.
...
종료된 작업은 안정적인 `deliveryId`를 가진 `operation_completion` 입력으로 요청한 대화와 에이전트에 한 번 전달된다. 호스트가 직접 완료 전달을 처리할 수 있고, 해당 기능이 없으면 런타임이 같은 에이전트의 새 턴으로 전달한다. 호스트가 완료 입력을 지속 가능한 상태로 인수하거나 런타임의 새 턴이 성공적으로 처리한 뒤 전달 완료를 확정한다. 재시작한 런타임은 저장소에서 대기, 승인, 실행 중, 전달 대기 작업을 복구해야 한다.
...
실행 결과의 `usage`는 흐름을 따라 실행한 에이전트들의 사용량을 합산한다. `finishReason`은 최종 출력에 도달한 에이전트의 종료 사유를 보존한다. 최종 출력이 여러 갈래이면 종료 사유가 모두 같을 때 그 값을 사용하고, 서로 다르면 `other`를 사용한다.
...
`error` 훅이 재시도를 요청하면 `target: model`은 현재 턴의 모델 호출 단계에서, `target: tool`은 실패한 도구 호출부터 이어서 실행한다. 사용자 입력은 같은 턴에 한 번 저장하며, 이미 성공한 도구 호출은 재실행하지 않는다. 도구 재시도가 성공하면 같은 모델 응답에서 아직 실행하지 않은 나머지 호출을 처리한다.
