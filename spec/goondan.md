...
`input`을 생략하거나 `asis`로 지정하면 문자열을 그대로 사용하고 다른 JSON 값은 JSON 문자열로 직렬화하여 사용자 메시지를 만든다. `input` 객체는 다음 규칙을 사용한다.
...
- `tool`은 호스트 도구 이름이다.
- `agent`는 같은 구성의 에이전트를 도구로 노출한다. 이 도구가 호출되면 호출 인수를 해당 에이전트의 입력으로 사용하고, 에이전트 출력 콘텐츠를 도구 결과로 반환한다. 하위 대화는 부모 대화, 부모 턴과 대상 에이전트의 조합으로 구분한다. 같은 부모 턴에서 같은 에이전트를 다시 호출하면 하위 대화를 이어가고, 다음 부모 턴에서는 새 하위 대화를 시작한다. 승인 후 실행할 때에도 승인 작업을 만든 부모 턴을 기준으로 삼는다.
...
실행 결과의 `usage`는 흐름을 따라 실행한 에이전트들의 사용량을 합산한다. 모델 응답을 훅에서 재시도하더라도 이미 발생한 사용량은 집계에 포함하며, 중첩 구성에서 발생한 사용량과 종료 사유도 바깥 흐름으로 전달한다. `finishReason`은 최종 출력에 도달한 에이전트의 종료 사유를 보존한다. 최종 출력이 여러 갈래이면 종료 사유가 모두 같을 때 그 값을 사용하고, 서로 다르면 `other`를 사용한다.
